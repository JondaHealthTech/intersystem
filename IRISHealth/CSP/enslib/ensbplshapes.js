/*
	...
	Definitions of Shape objects for BPL Editor
*/
...
function BPLEvent(diag, parent, type, name, x, y, isCopy)
{
	...
	ConstructBPLShape(this,diag.nextShapeNumber(),parent,type,name,x,y, isCopy);

	// add this to diagram
	diag.insertShape(this);
}

function BPLActivity(diag, parent, type, name, x, y, isCopy)
{
	...
	// add default values for new shapes...
	// (after construction is complete)
	switch (type) {
	case 'call':
		this.setProperty('Async',1);
		break;
	case 'sync':
		this.setProperty('Type','all');
		break;
	}

}

function BPLDecision(diag, parent, type, name, x, y, isCopy)
{
...
// flow
function BPLFlow(diag, parent, type, name, x, y, isCopy)
{
...
// scope
function BPLScope(diag, parent, type, name, x, y, isCopy)
{
...
// join
function BPLJoin(diag, parent, type, name, x, y, isCopy)
{
...
function BPLEvent_BuildShape(group)
{
	...
	text.setAttribute("style","clip-path:url(#clipPath_event);");
...
function BPLActivity_BuildShape(group)
{
	...
	if (this.Type == "sequence" || this.Type == "foreach" ||
		...
		ir.setAttribute("style","fill: none; stroke: blue; stroke-width: 1;");
		...
		gx.setAttribute("points", "-8,25 -8,10 8,10 8,25");
		gx.setAttribute("style","fill: none; opacity: 0.5; stroke: blue; stroke-width: 2;");
		...
		gx.setAttribute("style","fill: white; opacity: 0.5; stroke: none; stroke-width: 0;");
		group.appendChild(gx);
		...
		if (this.Type == "sequence" || this.Type == "compensationhandler" || this.Type == "catch" || this.Type == "catchall") {
			...
			gx.setAttribute("style","stroke: blue; stroke-width: 2;");
			group.appendChild(gx);
			...
			//gx.setAttribute("d", "M-4 16, A 4 4 0 1 1 0 20, L -3 20 l 3 -3, M -3 20, l 3 3");
			gx.setAttribute("d", "M-4 16, A 4 4 0 1 1 0 20, L -2 20, M -3 20 l 4 -4, M -3 19, l 4 4");
			gx.setAttribute("style","stroke: blue; stroke-width: 2; fill: none;");
			group.appendChild(gx);
	...
	if (this.Type == "delay") {
		...
		if ( (isAsync != 'undefined') && (isAsync != '') && ( (isAsync == 0) || (isAsync == '0') || (!isAsync) ) ) {
	...
	// text
	var text = document.createElementNS(SVGNS,"text");
	text.setAttribute("style","clip-path:url(#clipPath_activity);");
	text.setAttribute("id","shape_label_" + this.index);
	text.setAttribute("text-anchor", "middle");
	text.setAttribute("x", 0);
...
function BPLActivity_getInfo()
{
	...
	case 'code':
		...
		// xlate for Studio
		lang = (lang == 'objectscript') ? 'cache' : lang;
		state += 'Code'+insDelim2+'CODE_'+lang+insDelim2 + this.code + insDelim1;
		break;
	case 'sql':
		state += 'SQL'+insDelim2+'CODE_sql' + insDelim2 + this.code + insDelim1;
		break;
	case 'xslt':
		...
		state += 'Parameters'+insDelim2+'simple_table' + insDelim2 + parms + insDelim1;
		break;
	case 'call':
		...
		*/
		...
		state += 'Request'+insDelim2+'MESSAGE' + insDelim2 + msgstate + insDelim1;
		...
		state += 'Response'+insDelim2+'MESSAGE' + insDelim2 + msgstate + insDelim1;
...
function BPLActivity_setProperty(prop,value)
{
	...
	case 'Response':
		/*  serialized structure:
			...
			(assigns)  assign:property:type:key:value\x02
		*/
		// no undo

		// clear out old data
		if (prop == 'Request') {
			this.requestAssigns = new Array();
		}
		else {
			this.responseAssigns = new Array();
		}

		// pull apart incoming message
		var s = value.split(insDelim1a);
		for (var i = 0; i < s.length; i++) {
			var t = s[i].split(insDelim2);
			switch (t[0]) {
			case 'request':
				this.requestType = t[1];
				break;
			case 'response':
				this.responseType = t[1];
				break;
			case 'assign':
				...
				// insert
				if (prop == 'Request') {
					this.requestAssigns[this.requestAssigns.length] = assign;
				}
				else {
					this.responseAssigns[this.responseAssigns.length] = assign;
				}
				break;
			}
		}
...
function BPLDecision_BuildShape(group)
{
	...
	// text
	var text = document.createElementNS(SVGNS,"text");
	text.setAttribute("style","clip-path:url(#clipPath_decision);");
	text.setAttribute("id","shape_label_" + this.index);
	text.setAttribute("text-anchor", "middle");
	text.setAttribute("x", 0);
	text.setAttribute("y", 7);
