	...
	<xsl:template match="hl7:act" mode="eN-Status">
		...
		<xsl:if test="string-length($code)">
			<Status>
				<Code>
					<xsl:choose>
						<xsl:when test="$code = 'cancelled'">CA</xsl:when>
						<xsl:when test="$code = 'obsolete'">OB</xsl:when>
						<xsl:when test="$code = 'unavailable'">UN</xsl:when>
						<xsl:otherwise>AV</xsl:otherwise>
					</xsl:choose>
				</Code>
				<Description>
					<xsl:choose>
						<xsl:when test="$code = 'cancelled'">Deleted</xsl:when>
						<xsl:when test="$code = 'obsolete'">Obsolete</xsl:when>
						<xsl:when test="$code = 'unavailable'">Unavailable for patient care</xsl:when>
