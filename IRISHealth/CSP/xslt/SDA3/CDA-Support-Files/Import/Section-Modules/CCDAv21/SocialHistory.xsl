		...
		<xsl:if test="$IsNoDataSection='0' or $documentActionCode='XFRM'">
			<SocialHistories>
				...
				<xsl:if test="$IsNoDataSection='0'">
					...
					<xsl:apply-templates select="hl7:entry[not(.//@negationInd='true')]/hl7:observation[not(hl7:templateId/@root=$ccda-BirthSexObservation) and ($socialHistoryGenderIdentityImportMode = '1' or not(hl7:code/@code='76691-5'))]" mode="eSH-SocialHistory"/>
				</xsl:if>
			</SocialHistories>
		</xsl:if>
	</xsl:template>
