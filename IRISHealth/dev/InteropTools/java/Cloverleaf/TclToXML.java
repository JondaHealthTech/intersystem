...
public class TclToXML {
    public static void main(String[] args) throws Exception {
		...
		lexer.addErrorListener(TclErrorListener.INSTANCE);
		parser.removeErrorListeners();
		parser.addErrorListener(TclErrorListener.INSTANCE);

        ParseTree tree = parser.program(); // begin parsing at init rule
		if(new String("").equals(TclErrorListener.INSTANCE.toString())) {
			// only create the output file after we have parsed
			if ( outputFile!=null ) {
				PrintStream os = new PrintStream(new File(outputFile));
				System.setOut(os);
			}
		ParseTreeWalker.DEFAULT.walk(new TclParserBaseListener()
		{
	   	 	final String INDENT = "";
	    	int level = 0;
	    	@Override
